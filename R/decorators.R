...
`openeo-process` <- function(api, expr, ..., file) {
    ...
    # Scaffold JSON only when missing; never overwrite curated descriptors.
    # `_generated_by` marks auto-scaffolded files (see DEVELOPMENT.md).
    if (!file.exists(process_file)) {
        arg_names <- names(expr[[3]][[2]])
        arg_defaults <- unname(expr[[3]][[2]])
        process <- list(
            id = fn_name,
            `_generated_by` = "openeocraft",
